...
/proc/do_payday()
	...
	for(var/i in department_accounts)
		...
		if(!ED) // If no employer department found - payment is external
			...
			if(amount_to_pay <= EA.money)
				transfer_funds(EA, A, "Payroll Funding", "MSV-Explorator payroll system", amount_to_pay)
	...
	command_announcement.Announce("Hourly crew wages have been paid, please check your email for details. In total the crew of MSV-Explorator have earned [total_paid] credits, including [paid_external] credits from external sources.\n Please contact your Department Heads in case of errors or missing payments.", "Dispensation")
